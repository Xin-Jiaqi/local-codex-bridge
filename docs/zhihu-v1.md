...
手机这个界面的价值在于，它是为我服务的对话层。GPT 会把结果整理成人话，把失败原因讲清楚，把下一步的选择列出来。Codex 在本地做的那些原始输出，不用我逐个看。需要图的时候，GPT 在对话里生成示意图，或者把数据画成图，配着解释一起给我。v1.1.0 里开机自动拉起 Bridge 的 runtime、supervisor 和 launchd 代理已经写完，完整离线门禁通过；2026-08-14 在真实 host 上完整跑过 round-trip，自动恢复实机验证通过，v1.1.0 已于当天发布。旧版 Desktop 代理有 TCC 权限问题，运行时迁出 Desktop 正是这次改动的一部分。
...
文章里展开的这 8 个动作，对应我自己当前这只 Custom GPT 导入的工具集合：health 加 7 个 thread 操作。仓库里的 openapi.yaml 是公开模板，一共 9 个 operation，多出来的 /ready（operationId codexReady）返回和 /health 相同的 readiness 信息，主要给本机检查用；按 README 把完整 schema 导入 Custom GPT Actions，/ready 也会成为第 9 个 action。servers.url 是占位符；部署副本 openapi.ngrok.yaml 放真实域名，gitignored，不随仓库发布。所有 operation 都标了 x-openai-isConsequential: false，意思是 ChatGPT 调用前不弹确认。这个声明只是对 ChatGPT 的声明，安全边界由 key 认证、工作区沙箱和任务 cwd 守卫构成。
...
CODEX_HOME 指向独立 profile（默认 ~/.codex-deepseek），model、model_provider 和 DeepSeek provider 定义都在这个 profile 里，和日常终端的配置互不干扰；API key 不写入任何项目文件，只通过启动 bridge 的 shell 环境注入（DEEPSEEK_API_KEY），进程按需读取。

生命周期由 bash 脚本管：start、stop、status 三个入口，运行时的 PID 和锁放在 .runtime 目录，stop 只停自己启动的进程，复用 PID 前先做只读校验，校验失败只报告 stale 进程，绝不 kill。开机自启的 launchd per-instance 代理、前台 supervisor 和 pause marker 都在 v1.1.0 里写好，完整离线门禁通过；真实装载在 2026-08-14 的 host round-trip 里完成。开发中实测过一条运维规则：不要从 Bridge 沙箱内部发起重启，沙箱内拉起的任何进程都会继承 seatbelt profile，连 launchctl kickstart 拉起的 job 也一样；切换沙箱模式要在普通终端里 stop 再 start。
...
**沙箱模式。** 默认的 workspace-write 下，工作区根目录的 .git 是 protected path，只读，网络默认关闭，需要时用 BRIDGE_NETWORK_ACCESS=true 打开。这是 Codex 0.147.0 的既有权限语义，不是 bug；这个行为我用项目测试验证过。我拿默认配置跑需要 git 的任务时才发现，Bridge 自己没法 git add 和 commit。
...
三个修复合进当前工作区之后，完整离线门禁重跑通过，口径见下一节；实机上 local 和 maintenance 的 health 也重新验证过。
...
集成测试需要真实 app-server 和 DeepSeek key，不进 CI，可复现门禁以离线套件为准。最近一次完整手动实测是 2026-08-11：core 5/5、actions 7/7、HTTP API 11/11。
...
**版本时点。** v1.0.0 已经发布，仓库公开，BSD-3-Clause，tag 固定在 fa82e91、不移动。v1.1.0 已于 2026-08-14 发布，真实 host round-trip 通过：实例隔离、cwd 守卫、maintenance 维护窗口、bridge-workspace、运行时自动恢复（runtime + supervisor + per-instance LaunchAgent）、single-writer host-ops lock 和迁移脚本都在 1.1.0 线里，1.0.1 从未发布。文章里说的现在，都指这个时点。0.147.0 的 steer 排队还属于未文档化或 beta 的行为，升级 Codex 前要重新验证。
